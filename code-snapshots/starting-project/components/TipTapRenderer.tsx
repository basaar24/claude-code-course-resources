...
function renderNode(node: TipTapNode, key: number): React.ReactNode {
  ...
  switch (node.type) {
    ...
    case 'heading': {
      ...
      if (level === 2)
        return (
          <h2 key={key} className='mt-5 mb-1 text-xl font-bold'>
            {children}
          </h2>
        );
      return (
        <h3 key={key} className='mt-4 mb-1 text-lg font-semibold'>
          {children}
        </h3>
      );
    }
    case 'bulletList':
      return (
        <ul key={key} className='my-3 list-disc pl-6 space-y-1'>
          {children}
        </ul>
      );
    case 'orderedList':
      return (
        <ol key={key} className='my-3 list-decimal pl-6 space-y-1'>
          {children}
        </ol>
      );
    case 'listItem':
...
export default function TipTapRenderer({ contentJson }: { contentJson: string }) {
  ...
  return <div className='text-gray-900 dark:text-white'>{renderNode(doc, 0)}</div>;
}
