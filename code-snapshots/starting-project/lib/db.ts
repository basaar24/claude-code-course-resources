...
export function query<T>(sql: string, params?: BindingValue[]): T[] {
  return db.query(sql).all(...(params ?? [])) as T[];
}

export function get<T>(sql: string, params?: BindingValue[]): T | undefined {
  return db.query(sql).get(...(params ?? [])) as T | undefined;
}

export function run(sql: string, params?: BindingValue[]): void {
